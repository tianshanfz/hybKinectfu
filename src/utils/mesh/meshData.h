...
namespace ml{
	...
	public:
		...
		/*			const_noconst_iterator(const_noconst_iterator<false>& other) {
			...
			int resize(size_t numFaces, unsigned int faceValence = 3) {
				if (size() != 0)
				{
					cout<<"resize meshData error,not supported yet";
					return -1;
				}
			...
			void operator=(const std::vector<std::vector<unsigned int>>& other) {
				cout<<"inefficient function; should not actually been sued";
				clear();
				for (auto& f : other) {
		...
		void applyTransform(const Mat44& t) {
			for (size_t i = 0; i < m_Vertices.size(); i++) {
				float4 temp=t*make_float4(m_Vertices[i].x,m_Vertices[i].y,m_Vertices[i].z, 1.00);
				m_Vertices[i] = make_float3(temp.x,temp.y,temp.z);
			}
			for (size_t i = 0; i < m_Normals.size(); i++) {
				float4 temp=t*make_float4(m_Normals[i].x,m_Normals[i].y,m_Normals[i].z, 0.0);
				m_Normals[i] = make_float3(temp.x,temp.y,temp.z);
			}
		}
		...
		const Indices& getFaceIndicesNormals() const {
			if (!hasNormals()) cout<<"mesh does not have normals";
			...
			else cout << "vertex/normal mismatch";
		}
		const Indices& getFaceIndicesTexCoords() const {
			if (!hasTexCoords()) cout << "mesh does not have texcoords";
			...
			else cout << "vertex/texcoord mismatch";
		}
		const Indices& getFaceIndicesColors() const {
			if (!hasColors())	cout << "mesh does not have colors";
			...
			else cout << "vertex/color mismatch";
		}
		...
		std::vector<Point3f>	m_Vertices;			//vertices are indexed (see below)
		std::vector<Point3f>	m_Normals;			//normals are indexed (see below/or per vertex)
		std::vector<float2>	m_TextureCoords;	//tex coords are indexed (see below/or per vertex)
		std::vector<float4>	m_Colors;			//colors are not indexed (see below/or per vertex) 
		...
		void computeVertexNormals() {
			cout << "untested";
			if (m_FaceIndicesVertices.size() == 0) cout << "must be an indexed face set";
			m_Normals.clear();
			m_FaceIndicesNormals.clear();

			m_Normals.resize(m_Vertices.size(), make_float3(0, 0, 0));
			for (const auto& face : m_FaceIndicesVertices) {

				Point3f n = make_float3(0, 0, 0);
				unsigned int first = face[0];
				for (unsigned int i = 1; i < face.size() - 1; i++) {
					n += cross(m_Vertices[face[i]] - m_Vertices[first], m_Vertices[face[i + 1]] - m_Vertices[first]);
				}
				n = normalize(n);//normal of face
	...
	private:
		inline int3 toVirtualVoxelPos(const float3& v, FloatType voxelSize) {
			float3 temp=v *(1.0/ voxelSize) + make_float3(sign(v.x)*0.5,sign(v.y)*0.5,sign(v.z)*0.5);
			return make_int3((int)(temp.x),(int)(temp.y),(int)(temp.z));
		}
		//! returns -1 if there is no vertex closer to 'v' than thresh; otherwise the vertex id of the closer vertex is returned
		unsigned int hasNearestNeighbor(const int3& coord, SparseGrid3<std::list<std::pair<Point3f, unsigned int> > > &neighborQuery, const Point3f& v, FloatType thresh);
		...
		unsigned int hasNearestNeighborApprox(const int3& coord, SparseGrid3<unsigned int> &neighborQuery, FloatType thresh);
	};

	typedef MeshData<float>		MeshDataf;
	typedef MeshData<float>	MeshDatad;
