...
namespace ml {
	...
	void MeshIO<FloatType>::loadFromPLY(const std::string& filename, MeshData<FloatType>& mesh)
	{
		...
		if (!file.is_open())	cout<<"Could not open file " <<filename;

		// read header
		PlyHeader header(file);

		if (header.m_NumFaces == (unsigned int)-1) cout<<"no faces found";
		if (header.m_NumVertices == (unsigned int)-1) cout<<"no vertices found";
	...
	void MeshIO<FloatType>::loadFromOFF(const std::string& filename, MeshData<FloatType>& mesh)
	{
		...
		{	// ignore color
			...
			for (unsigned int i = 0; i < numV; i++) {
				...
				mesh.m_Vertices[i] = v;
				mesh.m_Colors[i] = c *(1/ 255.0);	//typically colors are stored in RGB \in [0;255]
			}
		}
	...
	void MeshIO<FloatType>::loadFromOBJ(const std::string& filename, MeshData<FloatType>& mesh)
	{
		...
		if (!fp) cout<<"Could not open file " << filename;
		...
		while (fscanf(fp, "%s", buf) != EOF) {
			if (strncmp(buf, "mtllib", strlen("mtllib")) == 0) {
				if (mesh.m_MaterialFile.size())cout<<"only a single mtllib definition allowed";
				fscanf(fp, "%s", buf);
				mesh.m_MaterialFile = filename + std::string(buf);
				...
				case 'v':
					...
					case '\0':
						...
						mesh.m_Vertices.push_back(make_float3(val[0], val[1], val[2]));

						if (match == 6) {  //we found color data
							mesh.m_Colors.push_back(mamke_float4(val[3], val[4], val[5], (FloatType)1.0));
						}
					...
					case 'n':
						...
						mesh.m_Normals.push_back(make_float3(val[0], val[1], val[2]));
					...
					case 't':
						...
						mesh.m_TextureCoords.push_back(make_float2(val[0], val[1]));
				...
				case 'f':
					...
					fscanf(fp, "%s", buf);
					{
						...
						if (sscanf(buf, "%d//%d", &idx[0][0], &idx[0][1]) == 2) {
							...
							cout<<filename <<": broken obj (face line invalid)";
							//skipLine( buf, OBJ_LINE_BUF_SIZE, fp);
						}

						if (n < 3) cout<<filename << ": broken obj (face with less than 3 indices)";
	...
	{
		if (!std::is_same<FloatType, float>::value) cout<<"only implemented for float, not for double";

		std::ofstream file(filename, std::ios::binary);
		if (!file.is_open())cout<<"Could not open file for writing " << filename;
		file << "ply\n";
		...
		if (mesh.m_Colors.size() > 0 || mesh.m_Normals.size() > 0) {
			...
			unsigned char* data = new unsigned char[vertexByteSize*mesh.m_Vertices.size()];
			size_t byteOffset = 0;
			for (size_t i = 0; i < mesh.m_Vertices.size(); i++) {
				...
				if (mesh.m_Colors.size() > 0) {
					uchar3 c = make_uchar3(mesh.m_Colors[i].x * 255, mesh.m_Colors[i].y * 255, mesh.m_Colors[i].z * 255);
