								...
								unsigned * updated_count)
{
	...
	for (unsigned z = 0; z < volume.resolution().z; z++)
	{
		...
		float depth = input_depthmap.get_data(screen_pos_depth.x,screen_pos_depth.y);
		if (depth == 0) continue;
		float normalz=	input_normalmap.get_data(screen_pos_depth.x, screen_pos_depth.y).z;
		...
		if(has_color)
		{
			...
			color= input_colormap.get_data(screen_pos_color.x, screen_pos_color.y);
...
{
	DepthfMap2D input_depth=CudaDeviceDataMan::instance()->_trunced_depth;
	Vector4fMap2D input_normal=CudaDeviceDataMan::instance()->_new_normals_pyramid[0];
	Color3uMap2D input_color=CudaDeviceDataMan::instance()->_raw_rgb;
	...
	unsigned *updated_voxel_count=CudaDeviceDataMan::instance()->_debug_count;
...
//	cudaMemcpy(&updated_voxel_count_cpu,updated_voxel_count,sizeof(int),cudaMemcpyDeviceToHost);
