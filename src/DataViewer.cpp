...
{
	Vector4fMap2D normals_view_map=normals_map.clone(CPU);
	...
	{
		arr_normals[col * 3 + 0 + row*cols * 3] = 255 * (normals_view_map.at(col, row).x - minValue.x) / (maxValue.x - minValue.x);
		arr_normals[col * 3 + 1 + row*cols * 3] = 255 * (normals_view_map.at(col, row).y - minValue.y) / (maxValue.y - minValue.y);
		arr_normals[col * 3 + 2 + row*cols * 3] = 255 * (normals_view_map.at(col, row).z - minValue.z) / (maxValue.z - minValue.z);
	}
...
{
	Color3uMap2D colors_view_map=colors_map.clone(CPU);
	int cols = colors_view_map.cols();
	int rows = colors_view_map.rows();
	cv::Mat image_show = cv::Mat(rows, cols, CV_8UC3, (void*)colors_view_map.ptr());
	//cv::Mat mImageBGR;
...
{
	DepthfMap2D depths_view_map=depths_map.clone(CPU);
	int cols = depths_view_map.cols();
	int rows = depths_view_map.rows();

	cv::Mat image_show;
	cv::Mat data_mat= cv::Mat(rows, cols, CV_32FC1, (void*)depths_view_map.ptr());
